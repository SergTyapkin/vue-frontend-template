...
export const Validators = {
  ...
  tg: {
    ...
    prettifyResult:  (str: string): string => str.replace(/https:\/\//i, '').replace(/t\.me\//i, '').replace('@', ''),
  },
  vk: {
    regExp: /^(https:\/\/)?(vk\.com\/|@)?(\w+\.)*\w+$/i,
    prettifyResult: (str: string): string => str.replace(/https:\/\//i, '').replace(/vk\.com\//i, '').replace('@', ''),
  },
  email: {
